...
x_transpose = x.T  # транспонирование матрицы х
xx = np.dot(x_transpose, x)  # умножение транспонированной на х
xx_inverse = np.linalg.inv(xx)  # возведение в -1 степень
xy = np.dot(x_transpose, y)  # умножение транспонированной на y
a = np.dot(xx_inverse, xy)  # результат

y_estimate = a[0] + a[1] * x[:, 1] + a[2] * x[:, 2]

print("Оценка Y (Y_estimate):\n", y_estimate)
print(y)
